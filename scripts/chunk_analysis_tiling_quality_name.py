...
class ChunkAnalysisTilingQualityName(AnalysisBase):
    ...
    def make_boxplot_quality_tiling(self):
        ...
        for self.metric in self.dataset_structure:
            ...
            for self.name in self.name_list:
                # Check files
                boxplot_path = self.boxplot_folder / f'boxplot_{self.metric}_{self.name}_quality.pdf'
    ...
    def make_boxplot_tiling_quality(self):
        ...
        for self.metric in self.dataset_structure:
            ...
            for self.name in self.name_list:
                # Check files
                boxplot_path = self.boxplot_folder / f'boxplot_{self.metric}_{self.name}_tiling.pdf'
                if boxplot_path.exists():
                    print(f'\t{boxplot_path} exists.')
                    continue
