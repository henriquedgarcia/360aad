...
class UserQuality(UserProjectionMetricsProps):
    ...
    def graphs2(self):
        ...
        for _ in loop_video_tiling():
            ...
            # result3[files'time_avg_total'].append(np.average(result4[files'time_total']))  # comparando entre usuários usamos o tempo médio
            # result3[files'time_avg_avg_sum'].append(np.sum(result4[files'time_avg_sum']))  # tempo médio sem paralelismo
            # result3[files'time_avg_avg'].append(np.average(result4[files'time_avg']))  # tempo total com decodificação paralela
            # result3[files'rate_total'].append(np.sum(result4[files'rate_sum']))  # taxa de bits sempre soma
            # result3[files'psnr_avg'].append(np.average(result4[files'PSNR_avg']))  # qualidade sempre é média
            # result3[files'ws_psnr_avg'].append(np.average(result4[files'WS-PSNR']))
            # result3[files's_psnr_avg'].append(np.average(result4[files'S-PSNR']))
